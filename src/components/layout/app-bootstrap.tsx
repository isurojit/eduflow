...
function Runtime({ children }: { children: React.ReactNode }) {
  ...
  const refreshNotifications = useEduFlowStore((state) => state.refreshNotifications);
  useEffect(() => hydrate(), [hydrate]);
  useEffect(() => {
    const timer = window.setInterval(refreshNotifications, 60_000);
    return () => window.clearInterval(timer);
  }, [refreshNotifications]);
  return <AuthGate>{children}<GlobalSearchDialog /><NotificationCenter /><MobileNav /><CloudSync /></AuthGate>;
}

export function AppBootstrap({ children }: { children: React.ReactNode }) {
  return <AuthProvider><Runtime>{children}</Runtime></AuthProvider>;
}
